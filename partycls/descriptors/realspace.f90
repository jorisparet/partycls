...
!  END FUNCTION qbarl
  ...
  !!!!!!!!!! PHYSICS-INSPIRED DESCRIPTOR (Jung et al.) !!!!!!!!!!
  SUBROUTINE physics_inspired_all(L, neigh, neigh_number, max_num_neigh, & 
                                 pos_0, pos_all, &
                                 spe_0, spe_all, beta, &
                                 epot_0, epot_all, &
                                 cell_0, cell_all, &
                                 box, Sk_i)
    ! parameters
    INTEGER(8), INTENT(in) :: neigh(:,:), neigh_number(:), max_num_neigh
    REAL(8), INTENT(in)    :: L(:), pos_0(:,:), pos_all(:,:), box(:)
    INTEGER(8), INTENT(in) :: spe_0(:), spe_all(:), beta
    REAL(8), INTENT(in)    :: epot_0(:), epot_all(:), cell_0(:), cell_all(:)
    ...
    REAL(8) :: epot_j(max_num_neigh+1), exp_j(max_num_neigh+1, SIZE(L))
    REAL(8) :: r_ij(SIZE(pos_0,1)), d_ij, exp_ij(SIZE(L))
    INTEGER(8) :: n, num_L, i, j, idx_j, nn_i, nj, spe_j
    LOGICAL    :: use_all
    ...
    DO i=1,SIZE(spe_0)
      ...
      cell_i = 0.0
      var_epot_i = 0.0
      ! loop over neighboring particles and vectorize on L
      nj = 0
      DO j=1,nn_i
        ! TODO: find a way to add i-i here...
        idx_j = neigh(i,j) + 1 ! python index shift
        spe_j = spe_all(idx_j)
        IF (spe_j == beta .OR. use_all) THEN
          ...
          cell_i = cell_i + cell_all(idx_j) * exp_ij
        END IF
      END DO
      ! include or exclude `i` based on its species
      IF (spe_0(i) == beta .OR. use_all) THEN
        nj = nj + 1
        epot_j(nj) = epot_0(i)
        exp_j(nj, :) = 1.0
        rho_i = rho_i + 1.0
        epot_i = epot_i + epot_0(i)
        cell_i = cell_i + cell_0(i)
      END IF
      ! normalization
      ...
      END DO
      ! fill S_k
      Sk_i(i,         1:1*num_L) = rho_i
      Sk_i(i, 1*num_L+1:2*num_L) = epot_i
      Sk_i(i, 2*num_L+1:3*num_L) = cell_i
      Sk_i(i, 3*num_L+1:4*num_L) = var_epot_i
    END DO
