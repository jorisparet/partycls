...
It can readily analyze trajectories generated by molecular dynamics simulation softwares, both within particle-based and multi-scale approaches (see *e.g.* @plimpton_fast_1995, @anderson_hoomd_2020, @Guzman_Tretyakov_Kobayashi_Fogarty_Kreis_Krajniak_Junghans_Kremer_Stuehn_2019), but also experimental data obtained with confocal microscope techniques [@Weeks_Crocker_Levitt_Schofield_Weitz_2000].
The code has already been used in the context of a recent publication [@paret_assessing_2020] and can be easily extended.
In particular, future versions will implement descriptors that depend on both space *and* time, to learn about the dynamics of the system as well.
Spatio-temporal clustering may provide valuable insight into the heterogeneous dynamics of amorphous materials [@berthier_dynamical_2011] and into particles' trajectories in more extreme states of matter [@Markidis_Peng_Podobas_Jongsuebchoke_Bengtsson_Herman_2020].
